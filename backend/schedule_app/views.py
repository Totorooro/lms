...
class LessonViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    permission_classes = [AllowAny]  # ← ОСТАВЬ ТОЛЬКО ЭТО

    def get_queryset(self):
        user = self.request.user

        # ЕСЛИ АВТОРИЗОВАН — ФИЛЬТРУЕМ ПО ГРУППЕ
        if user.is_authenticated and hasattr(user, 'group') and user.group:
            group = user.group
        else:
            # ДЛЯ ТЕСТА — ВОЗЬМЁМ ПЕРВУЮ ГРУППУ ИЛИ ВСЁ
            try:
                group = Group.objects.first()  # или group_id=1
            except:
